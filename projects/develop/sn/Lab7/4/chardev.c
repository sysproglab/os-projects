...
#define BUF_LEN 500		/* Максимальная длина сообщения */
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Shalyapin Nikita");
...
{	
	...
	if (length > 0) {
		sprintf(Message, "irq_counter = %d, esc_counter = %d, btn_right_counter = %d, btn_left_counter = %d\n", getIrqCounter(), getEscCounter(), getBtnRightCounter(), getBtnLeftCounter());
		Message_Ptr = Message;
	}
	/* Помещение данных в буфер о количестве прерываний с клавиатуры и нажати кнопки Esc */
	while (length && *Message_Ptr) {
     		put_user(*(Message_Ptr++), buffer++);
	  	length--;
	  	bytes_read++;
